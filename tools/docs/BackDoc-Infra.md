...
- [一、可观测性](#一可观测性)
- [二、数据库与事务](#二数据库与事务)
- [三、启动 / 关闭任务注册表（lifecycle）](#三启动-关闭任务注册表lifecycle)
- [四、异步任务队列（arq）—— 可选模块](#四异步任务队列arq-可选模块)
- [五、缓存](#五缓存)
- [六、迁移验证（MIGRATION_VERIFICATION）](#六迁移验证migration_verification)
- [七、运行环境与部署（0.9.8 补充）](#七运行环境与部署098-补充)
