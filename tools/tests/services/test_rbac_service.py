...
@pytest.fixture
def rbac_service(monkeypatch) -> RBACService:
    """注入 mock db 的真实 RBACService（ER-41：不再绕 __init__）。"""
...
async def test_update_role_returns_none_when_missing(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_update_role_delegates_to_repo(rbac_service, monkeypatch):
    svc = rbac_service
    role = MagicMock(id=1)
...
async def test_update_permission_returns_none_when_missing(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_update_permission_delegates_to_repo(rbac_service, monkeypatch):
    svc = rbac_service
    perm = MagicMock(id=2)
...
async def test_get_user_roles_empty_when_user_missing(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_get_user_roles_returns_user_roles(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock()
...
async def test_check_permission_user_missing_returns_false(rbac_service, monkeypatch):
    svc = rbac_service
    svc.rbac_repo.get_user_with_roles.return_value = None
...
async def test_check_permission_superuser_short_circuits(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock()
...
async def test_check_permission_aggregates_roles(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock()
...
async def test_inactive_role_does_not_grant_permissions(rbac_service, monkeypatch):
    """未启用角色不得带来权限。
...
async def test_get_all_roles_delegates(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_create_role_delegates(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_delete_role_delegates(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_get_all_permissions_delegates(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_create_permission_delegates(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_grant_role_to_user_idempotent(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock()
...
async def test_revoke_role_to_user_by_id(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock()
...
async def test_get_user_permissions_caches_result(rbac_service, monkeypatch):
    """第二次调用应命中缓存，不再查库。"""
...
async def test_grant_role_to_user_invalidates_cache(rbac_service, monkeypatch):
    """授予角色后应清除该用户权限缓存。"""
    cache = _real_memory_cache()
...
async def test_grant_admin_role_requires_superuser(rbac_service, monkeypatch):
    svc = rbac_service
...
async def test_grant_admin_role_allowed_for_superuser(rbac_service, monkeypatch):
    svc = rbac_service
    user = MagicMock(id=1, is_superuser=False)
    user.roles = []
    svc.rbac_repo.get_user_with_roles.return_value = user
