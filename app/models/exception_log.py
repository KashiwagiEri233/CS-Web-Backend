...
class ExceptionLog(Base):
    ...
    # 严重程度
    severity = Column(String(20), default="medium", nullable=False, index=True, comment="严重程度")
    priority = Column(String(20), default="normal", nullable=False, index=True, comment="优先级")
    
    # 关联信息
    parent_exception_id = Column(Integer, ForeignKey("exception_logs.id"), nullable=True, comment="父异常ID")
    related_incident_id = Column(String(64), nullable=True, index=True, comment="关联事件ID")
...
class ExceptionPattern(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    pattern_name = Column(String(100), nullable=False, unique=True, index=True, comment="模式名称")
...
class ExceptionAlert(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    alert_id = Column(String(64), nullable=False, unique=True, index=True, comment="告警ID")
    ...
    # 关联信息
    pattern_id = Column(Integer, ForeignKey("exception_patterns.id"), nullable=True, comment="模式ID")
    exception_log_ids = Column(JSON, nullable=True, comment="相关异常日志ID列表")
    
    # 告警状态
    status = Column(String(20), default="open", nullable=False, index=True, comment="状态")
    ...
    # 通知信息
    notification_sent = Column(Boolean, default=False, nullable=False, comment="是否已发送通知")
...
class ExceptionMetrics(Base):
    ...
    unique_tracebacks = Column(Integer, default=0, nullable=False, comment="唯一异常跟踪数")
    ...
    # 按异常类型分布
    application_exceptions = Column(Integer, default=0, nullable=False, comment="应用异常数")
    http_exceptions = Column(Integer, default=0, nullable=False, comment="HTTP异常数")
    validation_exceptions = Column(Integer, default=0, nullable=False, comment="验证异常数")
    database_exceptions = Column(Integer, default=0, nullable=False, comment="数据库异常数")
    
    # 按用户分布
    exceptions_from_authenticated_users = Column(Integer, default=0, nullable=False, comment="认证用户异常数")
    exceptions_from_anonymous_users = Column(Integer, default=0, nullable=False, comment="匿名用户异常数")
