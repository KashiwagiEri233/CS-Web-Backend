...
class ExceptionService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.metrics_collector = ExceptionMetrics()
    ...
    async def record_exception(
        ...
        """
        # 记录到日志系统
        log_record = self.logger.log_exception(exception, request_context, additional_data)
    ...
    async def detect_anomalies(
        ...
        # 检测异常
        return self.analyzer.detect_anomalies(log_records, baseline_window_hours, current_window_hours)
    ...
    async def store_metrics(
        ...
        metrics_record_data = {
            ...
            "client_errors_4xx": metrics_data.get("exception_types", {}).get("validation_exceptions", 0),
            "server_errors_5xx": metrics_data.get("exception_types", {}).get("application_exceptions", 0) + metrics_data.get("exception_types", {}).get("database_exceptions", 0),
            "application_exceptions": metrics_data.get("exception_types", {}).get("application_exceptions", 0),
            "http_exceptions": metrics_data.get("exception_types", {}).get("http_exceptions", 0),
            "validation_exceptions": metrics_data.get("exception_types", {}).get("validation_exceptions", 0),
            "database_exceptions": metrics_data.get("exception_types", {}).get("database_exceptions", 0),
            "exceptions_from_authenticated_users": metrics_data.get("user_metrics", {}).get("total_users_with_exceptions", 0),
            "exceptions_from_anonymous_users": metrics_data.get("user_metrics", {}).get("anonymous_exceptions", 0),
    ...
    async def resolve_exception(
        ...
        Returns:
            更新后的异常日志或None
        """
        return await self.log_repo.resolve_exception_log(log_id, resolved_by, resolution_notes)
    ...
    async def _process_exception_patterns(self, exception_log: ExceptionLog) -> None:
        ...
        try:
            ...
            # 检查是否需要创建告警
            if matched_pattern and matched_pattern.occurrence_count >= matched_pattern.alert_threshold:
    ...
    def _matches_pattern(self, exception_log: ExceptionLog, pattern: ExceptionPattern) -> bool:
        """检查异常日志是否匹配模式"""
        # 检查异常类型
        if pattern.exception_type and exception_log.exception_type != pattern.exception_type:
            return False
    ...
    async def _create_alert(self, exception_log: ExceptionLog, pattern: ExceptionPattern) -> None:
        """创建告警"""
        # 检查是否已存在相同模式的未解决告警
        ...
        alert_data = {
            ...
            "severity": "high" if pattern.occurrence_count >= pattern.alert_threshold * 2 else "medium",
