...
class ExceptionAnalyzer:
    ...
    def analyze_exception_patterns(
        self, logs: List[Dict[str, Any]], time_window_hours: int = 24
    ) -> Dict[str, Any]:
        """
        分析异常模式
        ...
        top_exceptions = sorted(
            exception_counts.items(), key=lambda x: x[1], reverse=True
        )[:10]
        top_error_codes = sorted(error_codes.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
    ...
    def detect_anomalies(
        ...
        # 分割时间窗口
        baseline_cutoff = datetime.now(timezone.utc).timestamp() - (
            baseline_window_hours * 3600
        )
        current_cutoff = datetime.now(timezone.utc).timestamp() - (
            current_window_hours * 3600
        )
        ...
        # 计算基准平均值
        baseline_rate = len(baseline_logs) / (
            baseline_window_hours - current_window_hours
        )
        ...
        for log in baseline_logs:
            error_code = log.get("error_code")
            if error_code:
                baseline_error_counts[error_code] = (
                    baseline_error_counts.get(error_code, 0) + 1
                )

        for log in current_logs:
            error_code = log.get("error_code")
            if error_code:
                current_error_counts[error_code] = (
                    current_error_counts.get(error_code, 0) + 1
                )
...
class ExceptionMetricsCollector:
    """异常指标收集器"""
    ...
    def collect_metrics(
        ...
        for log in window_logs:
            ...
            if "App" in exception_type or error_code:
                ...
                "validation" in error_code.lower()
                or "ValidationError" in exception_type
            ):
        ...
        return {
            ...
            "error_rates": {
                ...
                "other_error_rate": round(
                    (1 - error_rate_4xx - error_rate_5xx) * 100, 2
                ),
            },
            "exception_types": {
            ...
            "user_metrics": {
                ...
                "top_users_with_exceptions": sorted(
                    user_exceptions.items(), key=lambda x: x[1], reverse=True
                )[:5],
...
exception_metrics_collector = ExceptionMetricsCollector()
