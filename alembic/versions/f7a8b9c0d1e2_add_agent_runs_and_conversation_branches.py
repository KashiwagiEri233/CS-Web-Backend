...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "trigger_type", sa.String(length=20), nullable=False, server_default="chat"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["input_message_id"], ["chat_messages.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["output_message_id"], ["chat_messages.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        op.f("ix_conversations_root_conversation_id"),
        "conversations",
        ["root_conversation_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_conversations_forked_from_message_id"),
        "conversations",
        ["forked_from_message_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_chat_events_run_id"), "chat_events", ["run_id"], unique=False
    )
    op.drop_constraint("uq_chat_events_conv_seq", "chat_events", type_="unique")
    op.create_unique_constraint(
        "uq_chat_events_run_seq", "chat_events", ["run_id", "seq"]
    )
...
def downgrade() -> None:
    ...
    op.execute("""
        ...
        """)
    op.create_unique_constraint(
        "uq_chat_events_conv_seq", "chat_events", ["conversation_id", "seq"]
    )
    ...
    op.drop_index(
        op.f("ix_conversations_forked_from_message_id"), table_name="conversations"
    )
    op.drop_index(
        op.f("ix_conversations_root_conversation_id"), table_name="conversations"
    )
    op.drop_index(
        op.f("ix_conversations_parent_conversation_id"), table_name="conversations"
    )
    op.drop_constraint(
        "fk_conversations_forked_from_message_id", "conversations", type_="foreignkey"
    )
    op.drop_constraint(
        "fk_conversations_root_conversation_id", "conversations", type_="foreignkey"
    )
    op.drop_constraint(
