...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "question_snapshot", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.Column(
            "knowledge_tags", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.UniqueConstraint(
            "user_id", "question_id", name="ux_learning_wrong_answers_user_question"
        ),
    )
    for name, column in (
        ("user_id", "user_id"),
        ...
        op.create_index(
            op.f(f"ix_learning_wrong_answers_{name}"),
            "learning_wrong_answers",
            [column],
            unique=False,
        )
...
def downgrade() -> None:
    for name in ("review_due_at", "status", "question_id", "exam_id", "user_id"):
        op.drop_index(
            op.f(f"ix_learning_wrong_answers_{name}"),
            table_name="learning_wrong_answers",
        )
