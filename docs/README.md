...
| 文档 | 放什么内容 |
|---|---|
| `BackDoc-Arch.md` | 系统级架构：分层、中间件链、请求生命周期、关键不变量 |
| `BackDoc-Sec.md` | 鉴权（JWT/密码/黑名单）、异常体系、请求限流 |
| `BackDoc-Infra.md` | 日志 + 追踪/指标（可观测）、数据库/事务、启动关闭任务、队列、缓存 |
| `BackDoc-Mods.md` | 业务模块 API：认证 / 用户 / RBAC / 审计 |
| `BackDoc-Conv.md` | 编码规范、命名、质量红线、安全/错误处理约定 |
| `BackDoc-MigV.md` | Linux/PG 环境迁移验证指南 |

> 拿不准放哪时问一句"换一个完全不同的项目，这块还用得上吗？"——用得上 → 系统级（`BackDoc-Sec.md`/`BackDoc-Infra.md`）；
> 与具体业务实体（用户/角色…）绑定 → `BackDoc-Mods.md`。
...
- 新增**业务模块**时，在 `BackDoc-Mods.md` 对应节追加（或在 `docs/modules/<name>.md` 新建后登记到本文件索引）。
...
| 文档 | 覆盖代码 |
|---|---|
| [BackDoc-Sec.md](BackDoc-Sec.md) | 鉴权：`app/core/security.py`、`security_blacklist.py`、`middleware/rbac.py`、`password_compat.py`；异常：`app/core/exceptions/`；限流：`app/core/rate_limit/`、`middleware/rate_limit.py` |
...
| 文档 | 覆盖代码 |
|---|---|
| [BackDoc-Mods.md](BackDoc-Mods.md) | 认证：`app/api/v1/auth.py`、`services/auth_service.py`；用户：`app/api/v1/users.py`、`profile.py`；RBAC：`app/api/v1/rbac/`；审计：`app/api/v1/audit.py` |

### 迁移相关
| 文档 | 说明 |
|---|---|
| [BackDoc-MigV.md](BackDoc-MigV.md) | 数据层迁移的 Linux/PG 环境验证指南 |
...
- [ ] 新建模块 → `BackDoc-Mods.md` 登记或新建 `docs/modules/<name>.md` 并登记到本索引
